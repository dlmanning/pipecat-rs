...
- **[listen-and-respond](examples/listen-and-respond/)** — Voice conversational agent using local services. Mic → Whisper STT → Claude Code LLM → macOS Say TTS → Speaker. No third-party API keys needed.
...
- **[transcribe](examples/transcribe/)** — Transcribe audio using Silero VAD + local Whisper STT. Supports file input with optional speaker playback, or live microphone capture.
